...
public class GroqProvider : ILlmProvider
{
    ...
    {
        var key = await _preferencesService.GetAiApiKeyAsync("Groq", ct);
    ...
    public async Task<Result<ChatResult>> ChatAsync(ChatRequest request, CancellationToken ct)
    {
        return await _resiliencePolicy.ExecuteAsync(async () =>
        {
            try
            {
                ...
                {
                    return Result.Failure<ChatResult>("Invalid response from Groq API: empty choices or missing content");
                }
