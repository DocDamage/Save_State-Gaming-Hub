...
/// <summary>
/// Heuristic for detecting swimming state values in game memory.
/// IsSwimming values typically:
/// - Are boolean values (0 or 1)
/// - 1 when in water, 0 when on land
/// - Changes when entering/exiting water
/// - Usually has longer durations than other states
/// </summary>
public sealed class IsSwimmingHeuristic : IValueHeuristic
{
    public string Name => "Swimming State Detection";
    ...
    public double CalculateConfidence(DiscoveredValue value, List<ValueObservation> history)
    {
        ...
        // Swimming typically has transitions but not too many
        if (transitions >= 1 && transitions <= 4)
        {
            score += 0.25;
        }

        // Swimming usually has longer continuous durations
        if (maxContinuousOnes >= 5)
        {
            score += 0.2;
        }
