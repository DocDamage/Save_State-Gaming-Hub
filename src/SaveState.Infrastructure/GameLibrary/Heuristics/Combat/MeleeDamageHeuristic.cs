...
/// <summary>
/// Heuristic for detecting melee damage values in game memory.
/// Melee damage values typically:
/// - Are integers in range 1-50000
/// - Static or change with weapon/level
/// - Often lower than ranged damage in hybrid games
/// - Associated with close combat actions
/// </summary>
public sealed class MeleeDamageHeuristic : IValueHeuristic
{
    public string Name => "Melee Damage Detection";
    ...
    public double CalculateConfidence(DiscoveredValue value, List<ValueObservation> history)
    {
        ...
        double score = 0.0;
        int changes = 0;
        int meleeActionCorrelations = 0;

        // Check value range
        if (IsInMeleeDamageRange(value.CurrentValue))
        {
            score += 0.3;
        }
        ...
        for (int i = 1; i < history.Count; i++)
        {
            ...
            // Check for melee action correlation
            if (curr.RelatedAction == PlayerAction.Attacked && delta == 0)
            {
                meleeActionCorrelations++;
            }
        }

        // Melee damage should be relatively static (rare changes)
        if (history.Count > 1)
        {
        ...
        // Bonus for melee action correlations
        if (meleeActionCorrelations >= 2)
        {
            score += 0.1;
        }
    ...
    private static bool IsInMeleeDamageRange(object? value)
    {
