...
/// <summary>
/// Heuristic for detecting body/chassis damage percentage in driving/racing games.
/// Body damage values typically:
/// - Are floats (0.0-100.0) representing percentage
/// - Start at 0 for new vehicles
/// - Increase with collisions
/// - Can be repaired
/// </summary>
public sealed class BodyDamageHeuristic : IValueHeuristic
{
    public string Name => "Body Damage Detection";
    ...
    public double CalculateConfidence(DiscoveredValue value, List<ValueObservation> history)
    {
        ...
        bool hasCollisionIncrease = false;
        bool hasRepairDecrease = false;
        ...
        for (int i = 1; i < history.Count; i++)
        {
            ...
            // Damage increases with impacts
            if (currVal > prevVal)
            {
                var delta = currVal.Value - prevVal.Value;
                if (delta > 0 && delta < 50) // Collision damage
                {
                    hasCollisionIncrease = true;
                    score += 0.15;
                }
            }

            // Damage can be repaired (decreases)
            if (currVal < prevVal)
            {
                var delta = prevVal.Value - currVal.Value;
                if (delta > 10) // Significant repair
                {
                    hasRepairDecrease = true;
                    score += 0.1;
                }
            }
        ...
        // Bonus for collision detection
        if (hasCollisionIncrease)
            score += 0.15;

        // Bonus for repair detection
        if (hasRepairDecrease)
            score += 0.1;
