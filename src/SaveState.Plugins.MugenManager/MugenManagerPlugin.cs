...
/// </summary>
public class MugenManagerPlugin : IPlugin, IGameProvider, IMetadataScraper, IDisposable
{
    ...
    public async Task<Result<IReadOnlyList<Game>>> DiscoverGamesAsync(CancellationToken ct = default)
    {
        try
        {
            ...
            {
                Game.Create("MUGEN Character Pack 1", source: "MUGEN Manager"),
                Game.Create("MUGEN Character Pack 2", source: "MUGEN Manager"),
                Game.Create("MUGEN Tournament Edition", source: "MUGEN Manager")
            };

            foreach (var game in games)
            {
                // Metadata is stored via Tags collection instead of SetMetadata
                game.Tags.Add("MugenPack");
                game.Tags.Add("Source:MUGEN Archive");
                game.Tags.Add("Genre:Fighting");
            }
    ...
    public async Task<Result<Game>> GetGameDetailsAsync(string externalId, CancellationToken ct = default)
    {
        try
        {
            ...
            var game = Game.Create($"MUGEN: {externalId}", description: $"MUGEN character pack: {externalId}", source: "MUGEN Archive", sourceId: externalId);
            game.Tags.Add("MugenPack");
            game.Tags.Add("Genre:Fighting");
            game.Tags.Add("Developer:MUGEN Community");
...
versiondate = {_timeProvider.Now:MM-dd-yyyy}
mugenversion = 1.1
