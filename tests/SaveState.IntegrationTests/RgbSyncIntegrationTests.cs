...
{
    Task<Result<List<RgbDevice>>> DiscoverDevicesAsync();
    Task<Result<List<RgbDevice>>> GetConnectedDevicesAsync();
    Task<Result<List<RgbDevice>>> GetDevicesByTypeAsync(RgbDeviceType type);
    Task<Result<RgbDevice>> GetDeviceAsync(Guid deviceId);
    Task<Result<bool>> ConnectDeviceAsync(Guid deviceId);
    Task<Result<bool>> DisconnectDeviceAsync(Guid deviceId);

    Task<Result<bool>> ApplyEffectAsync(Guid deviceId, RgbEffect effect);
    Task<Result<bool>> ApplyEffectToMultipleAsync(List<Guid> deviceIds, RgbEffect effect);
    Task<Result<bool>> ClearEffectAsync(Guid deviceId);
    Task<Result<bool>> SetDeviceColorAsync(Guid deviceId, RgbColor color);
    Task<Result<bool>> SetDeviceBrightnessAsync(Guid deviceId, float brightness);
    Task<Result<bool>> SetEffectSpeedAsync(Guid deviceId, float speed);

    Task<Result<RgbProfile>> CreateProfileAsync(RgbProfile profile);
    Task<Result<RgbProfile>> GetProfileAsync(Guid profileId);
    Task<Result<List<RgbProfile>>> GetAllProfilesAsync();
    Task<Result<bool>> UpdateProfileAsync(RgbProfile profile);
    Task<Result<bool>> DeleteProfileAsync(Guid profileId);
    Task<Result<bool>> ApplyProfileAsync(Guid profileId);
    Task<Result<bool>> SetDefaultProfileAsync(Guid profileId);
    Task<Result<RgbProfile>> DuplicateProfileAsync(Guid profileId, string newName);
    Task<Result<string>> ExportProfileAsync(Guid profileId);
    Task<Result<RgbProfile>> ImportProfileAsync(string profileData);

    Task<Result<RgbSyncGroup>> CreateSyncGroupAsync(RgbSyncGroup group);
    Task<Result<RgbSyncGroup>> GetSyncGroupAsync(Guid groupId);
    Task<Result<bool>> UpdateSyncGroupAsync(RgbSyncGroup group);
    Task<Result<bool>> DeleteSyncGroupAsync(Guid groupId);
    Task<Result<bool>> AddDeviceToSyncGroupAsync(Guid groupId, Guid deviceId);
    Task<Result<bool>> RemoveDeviceFromSyncGroupAsync(Guid groupId, Guid deviceId);
    Task<Result<bool>> ApplySyncGroupEffectAsync(Guid groupId);

    Task<Result<bool>> SetGameStateTriggerAsync(GameStateRgbConfig config);
    Task<Result<List<GameStateRgbConfig>>> GetGameStateTriggersAsync();
    Task<Result<bool>> RemoveGameStateTriggerAsync(GameStateRgbTrigger trigger);
    Task<Result<bool>> TriggerGameStateAsync(GameStateRgbTrigger trigger);

    Task<Result<List<RgbProvider>>> GetAvailableProvidersAsync();
    Task<Result<bool>> EnableProviderAsync(string providerId);
    Task<Result<bool>> DisableProviderAsync(string providerId);
}
...
public record RgbEffect
{
    ...
    public RgbDirection Direction { get; set; }
...
public record RgbProvider
{
    public string Id { get; set; } = string.Empty;
    public string Name { get; set; } = string.Empty;
    public string Version { get; set; } = string.Empty;
    public bool IsEnabled { get; set; }
    public List<RgbDeviceType> SupportedDevices { get; set; } = new();
}
